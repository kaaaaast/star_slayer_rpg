...
/* Questa classe rappresenta le risorse della nave (vita, carburante etc...),
   creata per evitare di avere troppi metodi di gestione delle risorse nella classe ship, questo
...
public class ResourceStat {
    ...
    public ResourceStat (int max_value) {
        if (max_value <= 0) {
            throw new IllegalArgumentException("Il massimo deve essere impostato ad un valore maggiore di 0");
        }
    ...
    public void increase_resource_by (int amount) {
        if (amount < 0) {
    ...
    public void decrease_resource_by (int amount) {
        if (amount < 0) {
            throw new IllegalArgumentException();
        }
    ...
    public void set_max (int new_max_value) {
